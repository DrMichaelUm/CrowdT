...
#endif
    {
        ...
        public void Initialize()
        {
            ...
            if (!IsInitialized)
            {
                ...
                {
                    builder.AddProduct(product.ProductId, product.Type, new IDs
                {
        ...
        public void RestorePurchases(Action onRestoreFinished)
        {
            ...
            {
                Debug.Log("Store Extension Provider cannot be found, IAP havn't initialized properly or you are calling restore purhcase too early");
                return;
            }

            _storeExtensionProvider.GetExtension<IAppleExtensions>().RestoreTransactions((result) =>
            {
                Debug.Log(result ? "Succesfully restored IAP" : "Failed to restore IAP");
